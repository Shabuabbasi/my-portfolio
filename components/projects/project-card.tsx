...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <div className="relative p-6 w-full bg-background border border-border rounded-lg h-full flex flex-col">
      <div className="relative w-full h-[200px] flex-shrink-0 group">
        <Link href={project.websiteLink || `/projects/${project.id}`} target={project.websiteLink ? "_blank" : "_self"}>
          <Image
            className="rounded-lg border border-border object-cover transition-transform group-hover:scale-[1.02]"
            src={imageUrl}
