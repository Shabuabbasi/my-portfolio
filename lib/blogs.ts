...
/** Returns all blog slugs (file names without .md or .mdx) */
...
function getBlogPath(slug: string) {
  const mdxPath = path.join(BLOGS_DIR, `${slug}.mdx`);
  return fs.existsSync(mdxPath) ? mdxPath : path.join(BLOGS_DIR, `${slug}.md`);
}
...
/** Returns full blog post (metadata + raw MDX content) for a given slug */
