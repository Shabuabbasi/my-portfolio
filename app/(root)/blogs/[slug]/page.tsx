...
function getTableOfContents(content: string) {
  return content
    ...
    .flatMap((line) => {
      ...
      return [{ title, level: match[1].length, id: title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") }];
    });
}
  ...
  return (
    <ClientPageWrapper>
      ...
      <article className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground pb-6 border-b border-border">
              <address className="flex items-center gap-1.5 not-italic">
                ...
                <a
                  ...
                  className="hover:text-foreground transition-colors"
                >
                  {post.author || siteConfig.authorName}
                </a>
              </address>
        ...
        <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_12rem] lg:gap-10">
          ...
          {tableOfContents.length > 0 && (
            <aside className="hidden lg:block">
              <nav aria-label="Table of contents" className="sticky top-24 border-l border-border pl-4 text-sm">
                <p className="mb-3 font-medium text-foreground">On this page</p>
                <ol className="space-y-2 text-muted-foreground">
                  {tableOfContents.map((item) => <li key={item.id} className={item.level === 3 ? "pl-3" : ""}><a href={`#${item.id}`} className="hover:text-foreground">{item.title}</a></li>)}
                </ol>
              </nav>
            </aside>
          )}
